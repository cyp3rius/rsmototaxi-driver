...
import { Bell, BellOff, CloudOff, Eye, MapPinOff, RefreshCw, Zap } from 'lucide-react'
...
/**
 * Spec banner priority: impersonation → live trip → GPS → push → offline → sync.
 * One large primary + remaining as chips.
 */
export function SystemBanners() {
  ...
  if (me?.impersonation?.active) {
    primary = {
      ...
      body: me.impersonation.displayName
        ...
        : 'Tryb podglądu — zmiany są wyłączone.',
      tone: 'tint-accent',
  ...
  } else if (me?.liveTrip && me.dashboardState === 'C') {
    primary = {
      key: 'live',
      title: 'Kurs live w trakcie',
      body: 'Wróć do kursu, żeby śledzić GPS i zakończyć przejazd.',
      tone: 'bg-[var(--accent)] text-[var(--accent-on)]',
      actionLabel: 'Otwórz',
      href: '/app/trips/live',
    }
    ...
    primary = {
      ...
      body: 'Zmiany zapiszą się i zsynchronizują po połączeniu.',
      tone: 'tint-warning',
    }
  }

  if (primary?.key !== 'imp' && me?.impersonation?.active) {
    /* already primary */
  }
  if (primary?.key !== 'live' && me?.liveTrip && me.dashboardState === 'C') {
    chips.push({
      key: 'live',
      label: 'Kurs live',
      icon: Zap,
  ...
  return (
    <div className="space-y-2 px-5 pb-2.5">
      {primary ? (
        <div
          className={`rounded-[18px] px-4 py-3.5 ${primary.tone} ${
            primary.key === 'live' ? '' : 'text-[var(--text-primary)]'
          }`}
        >
          <p className={`text-[16px] font-semibold ${primary.key === 'live' ? '' : ''}`}>{primary.title}</p>
          <p
            className={`mt-1 text-[15px] leading-[21px] ${
              primary.key === 'live' ? 'opacity-85' : 'text-[var(--text-secondary)]'
            }`}
          >
            {primary.body}
          </p>
